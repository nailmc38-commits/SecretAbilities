...
public final class ThreatAnalyzer {
    public enum Level { CLEAR, GUARDED, ELEVATED, HIGH, CRITICAL }
    ...
    public record Snapshot(
        ...
        public String line() {
            ...
            if (hostileCount > 0) out.append(" // ").append(hostileCount).append(" mobs");
    ...
    public static Snapshot analyze(MinecraftClient client) {
        if (client == null || client.player == null || client.world == null) {
            return new Snapshot(Level.CLEAR, 0, 100, 0, -1, "", List.of(), "Area unavailable.");
        }
        ...
        } else if (p.getAir() < 140) {
            score += 3;
            reasons.add("low air");
        }

        score += Math.min(8, mobs.size());
        ...
        if (!mobs.isEmpty()) {
            ...
            if (nearestDistance < 3.0) {
                score += 5;
                reasons.add("melee range");
            } else if (nearestDistance < 6.0) {
                score += 3;
                reasons.add("very close mob");
        ...
        if (InventoryUtil.count(p, "totem_of_undying") == 0 && score >= 8) {
            score += 2;
            reasons.add("no totem");
        }

        Level level;
        if (score >= 19) level = Level.CRITICAL;
        else if (score >= 13) level = Level.HIGH;
        else if (score >= 8) level = Level.ELEVATED;
    ...
    public static int readiness(MinecraftClient client) {
        ...
        if (InventoryUtil.foodCount(p) == 0) score -= 12;
        if (InventoryUtil.count(p, "totem_of_undying") == 0) score -= 8;
        if (InventoryUtil.count(p, "shield") == 0) score -= 5;
